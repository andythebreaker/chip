...
fs.appendFileSync(`vector_add_vector_genED_M_${matrix_size_1}_${matrix_size_2}_V_${matrix_size_1}.sv`, hedder_10, (err) => {
    if (err) {
        console.error(err);
    } else {
...
for (let index = 0; index < matrix_size_2; index++) {
    ...
    //}
    tmp+=';\n';
    fs.appendFileSync(`vector_add_vector_genED_M_${matrix_size_1}_${matrix_size_2}_V_${matrix_size_1}.sv`, tmp, (err) => {
        if (err) {
            console.error(err);
        } else {
            console.log('File appended successfully.');
...
fs.appendFileSync(`vector_add_vector_genED_M_${matrix_size_1}_${matrix_size_2}_V_${matrix_size_1}.sv`, '\nendmodule\n', (err) => {
    if (err) {
        console.error(err);
    } else {
        console.log('File appended successfully.');
